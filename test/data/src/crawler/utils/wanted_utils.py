...
def wanted_login(driver:WebDriver):
    try:
        logger.info(f"전달 주소 : https://id.wanted.jobs/login?service=wanted&step=PASSWORD_ENTRY")
        driver.get("https://id.wanted.jobs/login?service=wanted&step=PASSWORD_ENTRY")
        time.sleep(5)
        logger.info(f"현재 주소 : {driver.current_url}")

        div = driver.find_element(By.ID, "__next")
        main = div.find_element(By.CLASS_NAME, "css-1i6vue")
        section = main.find_element(By.CLASS_NAME, "css-cvaybd")
        
        div2 = section.find_element(By.CLASS_NAME, "css-7qdx4z")
        div3 = div2.find_element(By.CLASS_NAME, "css-9bdxp9")
        div4 = div3.find_element(By.CLASS_NAME, "css-1qjqoce")
...
def wanted_search_company(driver: WebDriver, company_name: str, company_url: str):
    try:
        ...
        nav_bar_container = wait.until(
            EC.presence_of_element_located((By.CLASS_NAME, "NavBar_className__H6bPC"))
        )
        nav_bar = nav_bar_container.find_element(By.CLASS_NAME, "MainBar_MainBarNav__eLU8g")

        aside_container = wait.until(
            EC.presence_of_element_located((By.CLASS_NAME, "Aside_aside__vBaBA"))
        )
        ...
        search_btn = wait.until(
            EC.element_to_be_clickable((By.CLASS_NAME, 'Aside_searchButton__rajGo'))
        )
        search_btn.click()
        time.sleep(1)  # 검색창이 나타날 때까지 대기

        nav_search_bar = wait.until(
            EC.presence_of_element_located((By.ID, "nav_searchbar"))
        ).find_element(By.CLASS_NAME, "SearchBar_SearchBar__ZAr_1")
        ...
        time.sleep(2)  # 검색 결과가 로드될 때까지 대기
        ...
        for idx, card in enumerate(company_list):
            ...
            logger.info(f"searched_url : {searched_url}")
            ...
            text_container = company_info_container.find_element(By.CLASS_NAME, "wds-1abt75z")
            anchor = text_container.find_elements(By.TAG_NAME, "a")[-1]
...
def get_wanted_welfare(driver: WebDriver, company_name: str, company_url: str, company_bizno: str):
    ...
    try:
        wanted_login(driver)
        wanted_ids = wanted_search_company(driver, company_name, company_url)
...
async def get_wanted_welfare_async(driver: WebDriver, company_name: str, company_url: str, company_bizno: str):
    return await asyncio.get_event_loop().run_in_executor(
        executor, get_wanted_welfare, driver, company_name, company_url, company_bizno
    )