...
def save_to_mongo(collection, data):
    ...
    try:
        existing_data = collection.find_one({"company_name": data["company_name"]})
        if existing_data:
        ...
        logger.info(f"✅ {data['company_name']} 데이터가 MongoDB에 저장되었습니다.")
...
def async_mongo_client(db_name, collection_name):
    mongo_client = AsyncIOMotorClient("mongodb://ai:glab0110!!@local-mongodb:27017/?authSource=admin")
...
def get_or_create_company(company_name: str, homepage: str = None):
    ...
    if company:
        # logger.info(f"기존 회사 조회 성공: {company}")
