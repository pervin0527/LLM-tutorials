...
def add_new_page(collection, root_name, url, text):
    """
    특정 회사의 데이터를 확인하고, 존재하면 신규 페이지를 등록하는 함수.

    :param root_name: 회사명 (company)
    ...
    for page in pages:
        if page["url"] == url:
            logger.error(f"❌ {root_name}의 {url} 페이지가 이미 존재합니다.")
    ...
    logger.info(f"✅ {root_name}의 새로운 페이지 {url} 등록 성공.")
...
def update_page_content(collection, root_name, target_url, new_url=None, new_text=None):
    """
    특정 회사 데이터를 조회한 후, 해당 페이지의 URL 또는 텍스트를 수정하고 저장하는 함수.

    :param collection: MongoDB 컬렉션 객체
    :param root_name: 수정할 대상 회사명 (company)
    ...
    logger.info(f"✅ {root_name}의 {target_url} 페이지가 성공적으로 업데이트되었습니다.")

    return {"success": True, "message": "페이지 업데이트 성공", "updated_page": {"url": new_url or target_url, "text": new_text}}


def delete_page(collection, root_name, target_url):
    """MongoDB에서 특정 회사 데이터를 삭제"""
    collection.delete_one({"company": root_name, "pages": {"url": target_url}})
    logger.info(f"✅ {root_name}의 {target_url} 페이지가 삭제되었습니다.")
